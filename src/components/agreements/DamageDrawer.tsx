...
export const DamageDrawer: React.FC<DamageDrawerProps> = ({
  ...
  const handleSubmit = () => {
    ...
    const markerData = {
      ...
      photos: formData.photos.map(file => ({ name: file.name, url: '' })) // In real app, upload files first
    };
        ...
        <div className="px-4 pb-4 space-y-4 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              {formData.photos.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
                  {formData.photos.map((photo, index) => (
                    <div key={index} className="relative border rounded-lg p-2 flex items-center justify-between">
              ...
              {formData.severity === 'HIGH' && formData.photos.length === 0 && (
                <p className="text-sm text-red-500">At least one photo is required for high severity damage</p>
              )}
            </div>
          </div>
        </div>
