...
const SnapshotCard: React.FC<SnapshotCardProps> = ({ title, subtitle, details, onEdit }) => (
...
export const BillToSelector: React.FC<BillToSelectorProps> = ({
  ...
  const handleSelectChange = (id: string | string[]) => {
    ...
    onChange({ ...value, bill_to_id: selectedId });
  };

  const handlePaymentTermsChange = (id: string | string[]) => {
    const selectedId = Array.isArray(id) ? id[0] || '' : id;
    onChange({ ...value, payment_terms_id: selectedId });
  };
          ...
          <div className="space-y-4">
            {currentCustomerName && (
              <SnapshotCard
                ...
                details={[
                  { label: "Type", value: "Individual Customer" },
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.employee_ref || ''}
                  onChange={(e) => updateMeta('employee_ref', e.target.value)}
                  placeholder="Employee name or reference"
                />
              </div>
              <div className="space-y-2">
                <RequiredLabel>PO / RO Number</RequiredLabel>
                <Input
                  value={value.bill_to_meta?.po_ref || ''}
                  onChange={(e) => updateMeta('po_ref', e.target.value)}
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.policy_no || ''}
                  onChange={(e) => updateMeta('policy_no', e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.claim_no || ''}
                  onChange={(e) => updateMeta('claim_no', e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.adjuster_name || ''}
                  onChange={(e) => updateMeta('adjuster_name', e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.adjuster_phone || ''}
                  onChange={(e) => updateMeta('adjuster_phone', e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="email"
                  value={value.bill_to_meta?.adjuster_email || ''}
                  onChange={(e) => updateMeta('adjuster_email', e.target.value)}
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.voucher_ref || ''}
                  onChange={(e) => updateMeta('voucher_ref', e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.iata_code || ''}
                  onChange={(e) => updateMeta('iata_code', e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  value={value.bill_to_meta?.commission_pct || ''}
                  onChange={(e) => updateMeta('commission_pct', parseFloat(e.target.value) || 0)}
                  placeholder="0"
                  min="0"
                  max="100"
                  step="0.1"
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.tender_ref || ''}
                  onChange={(e) => updateMeta('tender_ref', e.target.value)}
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.auth_ref || ''}
                  onChange={(e) => updateMeta('auth_ref', e.target.value)}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <CustomerSelect
                  value={value.bill_to_id || ''}
                  onChange={handleSelectChange}
                  placeholder="Search event companies"
                />
                <FormError message={errors['bill_to_id']} />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.project_code || ''}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <CustomerSelect
                  value={value.bill_to_id || ''}
                  onChange={handleSelectChange}
                  placeholder="Search lease holders"
                />
                <FormError message={errors['bill_to_id']} />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.lease_no || ''}
                  onChange={(e) => updateMeta('lease_no', e.target.value)}
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={value.bill_to_meta?.relation || ''}
                  onChange={(e) => updateMeta('relation', e.target.value)}
  ...
  const billToOptions = [
    { value: 'CUSTOMER', label: 'Same as Customer', icon: User, id: 'billto-customer' },
    { value: 'CORPORATE', label: 'Corporate Account / Employer', icon: Building2, id: 'billto-corporate' },
    { value: 'INSURANCE', label: 'Insurance Company', icon: Shield, id: 'billto-insurance' },
    { value: 'AGENCY', label: 'Travel Agency / Tour Operator', icon: Plane, id: 'billto-agency' },
    { value: 'GOV', label: 'Government Department / Public Sector', icon: Building, id: 'billto-government' },
    { value: 'FLEET', label: 'Fleet Management Company', icon: Truck, id: 'billto-fleet' },
    { value: 'EVENT', label: 'Event Organizer / Production Company', icon: Calendar, id: 'billto-event' },
    { value: 'LEASE', label: 'Long-Term Lease Agreement Holder', icon: FileText, id: 'billto-leaseholder' },
    { value: 'THIRD_PARTY', label: 'Third-Party Individual (not the driver)', icon: User, id: 'billto-thirdparty' },
    { value: 'PARTNER', label: 'Partner Organization / Franchise', icon: Users, id: 'billto-partner' },
  ] as const;
            ...
            return (
              <div key={option.value} className="space-y-2">
                ...
                <Collapsible open={isSelected}>
                  <CollapsibleContent 
                    className={cn(
                      "ml-6 p-4 border rounded-lg bg-muted/30",
                      `panel-${option.id}`
                    )}
                  >
                      ...
                      <CollapsibleContent className="mt-3 space-y-4">
                        <div className="grid gap-4 md:grid-cols-2">
                          <div className="space-y-2">
                            ...
                            <Input
                              value={value.bill_to_meta?.tax_reg || ''}
                              onChange={(e) => updateMeta('tax_reg', e.target.value)}
                              placeholder="Tax ID / VAT number"
                            />
                          </div>
                          <div className="space-y-2 md:col-span-2">
                            <Label>Notes for AR</Label>
                            <Textarea
                              value={value.bill_to_meta?.notes_ar || ''}
                              onChange={(e) => updateMeta('notes_ar', e.target.value)}
                              placeholder="Notes for accounts receivable"
                              rows={2}
                            />
                          </div>
                        </div>
