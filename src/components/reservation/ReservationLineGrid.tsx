...
export const ReservationLineGrid: React.FC<ReservationLineGridProps> = ({
  ...
  if (lines.length === 0) {
    return <div className="text-center py-8 text-muted-foreground bg-slate-50 rounded-3xl">
        No reservation lines added yet. Fill out vehicle information and click "Add Line" to get started.
      </div>;
  }
  return <div id="grid-reservation-lines" className="border rounded-lg">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="w-12">
              <Checkbox checked={selectedLines.length === lines.length && lines.length > 0} onCheckedChange={handleSelectAll} />
            </TableHead>
        ...
        <TableBody>
          ...
          return <TableRow key={line.id} className={cn(isSelected && "bg-muted/50")} data-id={`row-${line.lineNo}`}>
                <TableCell>
                  <Checkbox checked={isSelected} onCheckedChange={checked => handleSelectLine(line.id, Boolean(checked))} />
                </TableCell>
                <TableCell className="font-medium">{line.lineNo}</TableCell>
                <TableCell>
                  <Select value={line.reservationTypeId} onValueChange={value => onLineUpdate(line.id, {
                ...
                <TableCell>
                  <Select value={line.vehicleClassId} onValueChange={value => onLineUpdate(line.id, {
                ...
                <TableCell>
                  <Select value={line.vehicleId} onValueChange={value => onLineUpdate(line.id, {
                vehicleId: value
              })}>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    {line.drivers && line.drivers.length > 0 ? <>
                        ...
                        {line.drivers.length > 1 && <Badge variant="secondary" className="text-xs">
                ...
                <TableCell>
                  {editingCell?.lineId === line.id && editingCell?.field === 'lineNetPrice' ? <Input type="number" step="0.01" value={editValue} onChange={e => setEditValue(e.target.value)} onBlur={handleCellBlur} onKeyDown={handleKeyDown} className="w-24" autoFocus /> : <div className="min-h-[40px] flex items-center px-2 cursor-pointer hover:bg-muted/50 rounded font-mono" onDoubleClick={() => handleCellDoubleClick(line.id, 'lineNetPrice', line.lineNetPrice)}>
                      {formatCurrency(line.lineNetPrice)}
                    </div>}
                </TableCell>
                <TableCell>
                  {editingCell?.lineId === line.id && editingCell?.field === 'taxValue' ? <Input type="number" step="0.01" value={editValue} onChange={e => setEditValue(e.target.value)} onBlur={handleCellBlur} onKeyDown={handleKeyDown} className="w-24" autoFocus /> : <div className="min-h-[40px] flex items-center px-2 cursor-pointer hover:bg-muted/50 rounded font-mono" onDoubleClick={() => handleCellDoubleClick(line.id, 'taxValue', line.taxValue)}>
                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" onClick={() => setShowMoreModal(line)} className="h-8 w-8 p-0" id={`icon-show-more-${line.lineNo}`}>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => onLineRemove(line.id)} className="text-destructive">
      ...
      <LineDetailsModal line={showMoreModal} isOpen={!!showMoreModal} onClose={() => setShowMoreModal(null)} onSave={onLineUpdate} onDelete={onLineRemove} />
    </div>;
};