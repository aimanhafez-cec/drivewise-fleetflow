...
import { TrendingUp, Clock, AlertTriangle, Car, CalendarClock, Users } from "lucide-react";
...
export const PlannerKPIs: React.FC<PlannerKPIsProps> = ({
  events
}) => {
  ...
  const dueOut = events.filter(e => e.start.startsWith(todayStr) && (e.kind === "RESERVATION" || e.kind === "AGREEMENT")).length;
  const dueIn = events.filter(e => e.end.startsWith(todayStr) && e.kind === "AGREEMENT").length;
  const overdue = events.filter(e => new Date(e.end) < today && e.status === "open").length;
  const idle = events.filter(e => e.kind === "RESERVATION" && e.status === "open").length;
  const kpis = [{
    id: "utilization",
