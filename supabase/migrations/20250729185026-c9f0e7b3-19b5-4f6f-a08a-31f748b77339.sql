...
CREATE TYPE public.vehicle_status AS ENUM ('available', 'rented', 'maintenance', 'out_of_service', 'reserved');
CREATE TYPE public.reservation_status AS ENUM ('pending', 'confirmed', 'checked_out', 'completed', 'cancelled');
CREATE TYPE public.agreement_status AS ENUM ('active', 'completed', 'terminated', 'pending_return');
CREATE TYPE public.payment_status AS ENUM ('pending', 'processing', 'completed', 'failed', 'refunded');
CREATE TYPE public.damage_type AS ENUM ('scratch', 'dent', 'crack', 'missing_part', 'interior_damage', 'other');
CREATE TYPE public.user_role AS ENUM ('admin', 'fleet_manager', 'rental_agent', 'customer', 'maintenance', 'finance');
...
-- Vehicles - readable by all authenticated users, modifiable by staff
...
-- Reservations - customers see their own, staff see all
...
-- Agreements - similar to reservations
CREATE POLICY "agreements_customer" ON public.agreements FOR SELECT TO authenticated 
USING (customer_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid()));
CREATE POLICY "agreements_staff" ON public.agreements FOR ALL TO authenticated USING (true);

-- All other tables - staff access
...
CREATE TRIGGER update_traffic_tickets_updated_at BEFORE UPDATE ON public.traffic_tickets
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
